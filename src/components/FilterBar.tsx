          ...
          {categories.map(cat => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 sm:gap-2.5 px-3 sm:px-4 py-2 rounded-full border whitespace-nowrap text-xs sm:text-sm font-medium transition-colors min-h-[40px] ${
                  isActive
              ...
              >
                <span>{cat.emoji}</span>
                <span>{label}</span>
                <span className={`text-[10px] sm:text-xs px-1.5 py-0.5 rounded-full ${isActive ? 'bg-white/20' : 'bg-white/10'}`}>{count}</span>
              </button>
            );
          })}
        </div>
