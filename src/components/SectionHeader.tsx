...
import { categoryDescriptions } from '@/data/projectTranslations';
...
const SectionHeader = ({ category, count }: SectionHeaderProps) => {
  ...
  const desc = categoryDescriptions[language]?.[category.id] || category.description;
    ...
    <div className="flex items-center justify-between border-b border-white/10 mb-6 sm:mb-8 lg:mb-10 pb-4 sm:pb-5 lg:pb-6 gap-3">
      <div className="flex items-center gap-3 sm:gap-4 min-w-0">
        ...
        <div className="min-w-0">
          ...
          {desc && <p className="text-xs sm:text-sm lg:text-base text-hub-muted truncate">{desc}</p>}
        </div>
      </div>
